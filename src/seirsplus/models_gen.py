...
class SEIRSModel():
    ...
    def __init__(self, initN, beta, sigma, gamma, alpha, delta, xi=0, mu_I=0, mu_0=0, nu=0, p=0,
                        beta_D=None, sigma_D=None, gamma_D=None, alpha_D=None, delta_D=None, mu_D=None, 
                        theta_E=0, theta_I=0, psi_E=0, psi_I=0, q=0,
                        initE=0, initI=10, initD_E=0, initD_I=0, initH=0, initR=0, initD=0):
        ...
        self.numH       = numpy.array([float(initH)])
        self.numR       = numpy.array([float(initR)])
        self.numD       = numpy.array([float(initD)])
        self.numS       = numpy.array([self.N[-1] - self.numE[-1] - self.numI[-1] - self.numD_E[-1] - self.numD_I[-1] -self.numH[-1] - self.numR[-1] - self.numD[-1]])
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    @staticmethod
    def system_dfes(t, variables, beta, sigma, gamma, alpha, delta, xi, mu_I, mu_0, nu,
                                  beta_D, sigma_D, gamma_D, mu_D, theta_E, theta_I, psi_E, psi_I, q):

        S, E, I, D_E, D_I, H, R, D = variables    # varibles is a list with compartment counts as elements 
        ...
        return [dS, dE, dI, dDE, dDI, dH, dR, dD]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_epoch(self, runtime, dt=0.1):
        ...
        init_cond       = [self.numS[-1], self.numE[-1], self.numI[-1], self.numD_E[-1], self.numD_I[-1], self.numH[-1], self.numR[-1], self.numD[-1]]

        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Solve the system of differential eqns:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        solution        = scipy.integrate.solve_ivp(lambda t, X: SEIRSModel.system_dfes(t, X, self.beta, self.sigma, self.gamma, self.alpha, self.delta, self.xi, self.mu_I, self.mu_0, self.nu,
        ...
        self.numH       = numpy.append(self.numH, solution['y'][5])
        self.numR       = numpy.append(self.numR, solution['y'][6])
        self.numD       = numpy.append(self.numD, solution['y'][7])
