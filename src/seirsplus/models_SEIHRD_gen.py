...
class SEIHRDModel():
    ...
    Params: beta    Rate of transmission (exposure) 
            ...
            gamma   Rate of detection (upon infection) 
            alpha   Rate of recovery (after hospitalisation)
            delta   Rate of fatality (after hospitalisation)
            kappa   Fraction of population getting recovered
            ...
            initHr  Init number of people in hospitalisation that will move into recoverey
            initHd  Init number of people in hospitalisation that will pass away
    ...
    def __init__(self, initN, beta, sigma, gamma, alpha, delta, kappa, xi=0, mu_I=0, mu_0=0, nu=0, p=0,
                        beta_D=None, sigma_D=None, gamma_D=None, alpha_D=None, delta_D=None, mu_D=None, 
                        theta_E=0, theta_I=0, psi_E=0, psi_I=0, q=0,
                        initE=0, initI=10, initD_E=0, initD_I=0, initHr=0, initHd=0, initR=0, initD=0):
        ...
        self.alpha_D  = alpha_D if alpha_D is not None else self.alpha
        self.delta_D  = delta_D if delta_D is not None else self.delta
        ...
        self.numE       = numpy.array([float(initE)])
        self.numI       = numpy.array([float(initI)])
        self.numD_E     = numpy.array([float(initD_E)])
        self.numD_I     = numpy.array([float(initD_I)])
        self.numHr      = numpy.array([float(initHr)])
        self.numHd      = numpy.array([float(initHd)])
        self.numR       = numpy.array([float(initR)])
        self.numD       = numpy.array([float(initD)])
        self.numS       = numpy.array([self.N[-1] - self.numE[-1] - self.numI[-1] - self.numD_E[-1] - self.numD_I[-1] -self.numHr[-1] - self.numHd[-1] - self.numR[-1] - self.numD[-1]])
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    @staticmethod
    def system_dfes(t, variables, beta, sigma, gamma, alpha, delta, kappa, xi, mu_I, mu_0, nu,
                                  beta_D, sigma_D, gamma_D, mu_D, theta_E, theta_I, psi_E, psi_I, q):

        S, E, I, D_E, D_I, Hr, Hd, R, D = variables    # varibles is a list with compartment counts as elements 
        ...
        dHr = gamma*I*(kappa) - alpha*Hr

        dHd = gamma*I*(1 - kappa) - delta*Hd

        dR  = alpha*Hr

        dD  = delta*Hd

        return [dS, dE, dI, dDE, dDI, dHr, dHd, dR, dD]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_epoch(self, runtime, dt=0.1):
        ...
        init_cond       = [self.numS[-1], self.numE[-1], self.numI[-1], self.numD_E[-1], self.numD_I[-1], self.numHr[-1], self.numHd[-1], self.numR[-1], self.numD[-1]]

        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Solve the system of differential eqns:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        solution        = scipy.integrate.solve_ivp(lambda t, X: SEIHRDModel.system_dfes(t, X, self.beta, self.sigma, self.gamma, self.alpha, self.delta,  self.kappa, self.xi, self.mu_I, self.mu_0, self.nu,
        ...
        self.numHr      = numpy.append(self.numHr, solution['y'][5])
        self.numHd      = numpy.append(self.numHd, solution['y'][6])
        self.numR       = numpy.append(self.numR, solution['y'][7])
        self.numD       = numpy.append(self.numD, solution['y'][8])
...
#             print("t = %.2f" % self.t)
            if(verbose):
                ...
                print("\t Hr   = " + str(self.numHr[-1]))
                print("\t Hd   = " + str(self.numHd[-1]))
                print("\t R   = " + str(self.numR[-1]))
                print("\t F   = " + str(self.numD[-1]))
