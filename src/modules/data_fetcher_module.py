...
class DataFetcherModule(object):
    ...
    def get_regional_metadata(region_type, region_name,
                              data_source=DataSource.tracker_district_daily,
                              file_path="../data/regional_metadata.json"):
        data_fetcher = DataFetcherFactory.get_data_fetcher(data_source)
        metadata = data_fetcher.get_regional_metadata(region_type, region_name, file_path)
        return metadata
